...
class ProgressBar(Callback):
    ...
    def __exit__(self, type, val, tb):
        ...
        if self.validiation_bar is not None:
            self.validiation_bar.close()
    ...
    def on_epoch_begin(self, epoch, logs=None):
        self.training_bar = tqdm(total=self.train_logs['num_batches'], unit=' batch', desc=f'Training Epoch {epoch + 1}', position=0)
    ...
    def on_validation_begin(self, logs=None):
        self.validiation_bar = tqdm(total=self.train_logs['val_num_batches'], unit=' batch', desc='Validation',
                                    leave=True, position=1)

    def on_validation_epoch_begin(self, logs=None):
        self.validiation_bar.update(1)
